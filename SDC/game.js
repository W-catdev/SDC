...
function damageEnemy(e, dmg, fromAngle) {
    ...
    const hitAngle = Math.atan2(G.player.y - e.y, G.player.x - e.x);
    const relativeAngle = Math.abs(normalizeAngle(hitAngle - (e.facing || 0)));
...
function update(now) {
    ...
    if (p.vaulting && p.vaultTarget) {
        ...
        speed = p.speed;
        // 冲刺
        isSprinting = (G.keys['shift']) && p.stamina > 5 && mag > 0;
        if (isSprinting) {
            speed *= p.sprintMul;
            ...
            for (let bi = 0; bi < G.barrels.length; bi++) {
                ...
                if (d2 < (br.r + 4) ** 2) {
                    ...
                    if (br.hp <= 0) {
                        ...
                        if (br.type === 'gas') {
                            G.fireZones.push({
                                x: br.x, y: br.y, radius: 90, life: 4, age: 0,
                                tickInterval: 0.4, tickTimer: 0, dmg: 12
                            });
                        }
                    }
                    consumed = true;
    ...
    for (const e of G.enemies) {
        ...
        if (e.state === 'patrol' || e.state === 'alert') {
            for (const snd of G.audioEvents) {
                ...
                if (ds < hearR && !e._heardThis) {
                    ...
                    e.alertTimer = Math.max(e.alertTimer || 0, 6);
        ...
        if (e.state === 'patrol' || e.state === 'alert') {
            for (const corpse of G.enemies) {
                if (corpse.state !== 'dead' || corpse === e) continue;
                const key = `${Math.round(corpse.x)}_${Math.round(corpse.y)}`;
                if (G.discoveredCorpses[key]) continue;
        ...
        if (e.state === 'patrol') {
            ...
            // 远距离丢失目标
            if (toPlayer > e.loseRange && e.state !== 'alert') {
                e.state = 'patrol';
                e.patrolTarget = null;
                e.coverTarget = null;
                e.moveState = 'approach';
            ...
            // === alert（调查）状态：向 investigateTarget 移动并四处张望 ===
            if (e.state === 'alert' && e.investigateTarget) {
                if (canSeePlayer) {
                    e.state = 'chase';
                    e.alertTimer = e.alertDuration;
                    e.moveState = 'approach';
    ...
    if (dd._windowTimer > 8) {
        ...
        let targetLevel = 1.0;
        if (kills >= 3 && dmgTaken < 40) targetLevel = 1.3;
        else if (kills >= 2) targetLevel = 1.1;
        else if (dmgTaken > 120) targetLevel = 0.75;
...
function render() {
    ...
    for (const br of G.barrels) {
        if (br.exploded) continue;
        ctx.save();
        const sh = (br.shake || 0) > 0 ? (Math.random() - 0.5) * 3 : 0;
        const sx = br.x + sh;
        const sy = br.y + sh;
        // 阴影
...
function drawPlayer(p) {
    ...
    // 2.4 手臂（按武器状态不同形态）
    const w = p.weapons[p.currentWeapon];
    // === v1.1.x 武器动画状态（切枪/开火/装填/近战）===
    const nowT = performance.now();
    let armRetract = 0;      // 枪身后退量（开火后坐）
    let weaponRaise = 0;     // 武器下沉量（切枪/装填）
    let weaponBob = 0;       // 垂直微抖
    if (G.weaponAnim && G.weaponAnim.startTime) {
        const dta = nowT - G.weaponAnim.startTime;
        if (dta < G.weaponAnim.duration) {
            const t = dta / G.weaponAnim.duration;
            if (G.weaponAnim.state === 'fire') {
                ...
                armRetract = Math.sin(t * Math.PI) * -8; // 前刺
            }
        }
    }
    // 移动微抖
    const speedNow = Math.hypot(p.vx || 0, p.vy || 0);
    if (speedNow > 20) weaponBob = Math.sin(G.time * 10) * 0.6;
    ...
    if (w && !w.melee) {
        ...
        const gy = p.y + Math.sin(a) * baseReach + weaponBob + weaponRaise;
        // 手臂阴影
